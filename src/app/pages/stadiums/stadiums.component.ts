...
@Component({
  ...
  template: `
    <div class="px-20 my-10 flex justify-between">
      <div>
    ...
    <div class="px-20 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
      @for (stadium of stadiums; track $index) {
        <div class="border border-neutral-200 rounded-xl overflow-hidden shadow-md">
          <div class="relative shadow-md">
            <div class="bg-white text-neutral-700 rounded-lg text-sm shadow-md font-semibold px-3 absolute left-1.5 top-2">
              StadiumId: {{ stadium.stadiumId }}
            </div>
            <img [src]="stadium.image" alt="STADIUM-image" class="w-full h-40 object-cover">
          </div>
          <div class="px-4 py-3">
            <p class="font-semibold mb-2">{{ stadium.name }}</p>
