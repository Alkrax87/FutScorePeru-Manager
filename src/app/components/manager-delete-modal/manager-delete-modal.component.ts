...
@Component({
  selector: 'app-manager-delete-modal',
  ...
  template: `
    <div class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50 select-none">
      <div class="bg-white p-5 rounded-xl w-full max-w-sm text-justify">
        <h3 class="text-xl font-semibold mb-2">Delete Manager</h3>
        <p class="mb-6 text-neutral-500">
          Are you sure you want to delete <span class="text-black font-bold">{{ deletedManager.name }}</span>? This will permanently remove manager data and cannot be undone.
        </p>
...
export class ManagerDeleteModalComponent {
  @Input() deletedManager!: Manager;
