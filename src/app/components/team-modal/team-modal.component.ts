  ...
  template: `
    <div class="bg-black bg-opacity-70 fixed inset-0 z-50 flex justify-center items-center select-none px-3">
      <div class="bg-crimson rounded-3xl overflow-hidden w-full max-w-2xl">
        <div class="p-5">
          @if (team) {
            <h3 class="text-white text-xl font-semibold">Edit Team</h3>
            <p class="text-neutral-200 text-sm">Update the team details below.</p>
          } @else {
            <h3 class="text-white text-xl font-semibold">Add Team</h3>
        ...
        <form [formGroup]="form" (ngSubmit)="save()" class="bg-white px-5 pb-5 pt-2">
          <div class="flex flex-col gap-4 my-4">
            ...
            <!-- Background -->
            <div>
              <label for="background" class="relative">
                <input id="background" type="text" formControlName="background" placeholder="" autocomplete="false" class="bg-white text-neutral-700 border focus:border-crimson focus:text-crimson h-12 cursor-text px-5 py-2 peer w-full rounded-full shadow-sm duration-100 outline-none">
                <span class="bg-white text-neutral-400 peer-focus:text-crimson cursor-text flex items-center -translate-y-6 absolute inset-y-0 start-3 px-2 text-xs font-semibold transition-transform peer-placeholder-shown:translate-y-0 peer-focus:-translate-y-6">Background</span>
              </label>
            </div>
            <div class="flex gap-4">
              <!-- Group First Phase -->
...
export class TeamModalComponent {
  ...
  save() {
    ...
    const formTeam = this.form.value as Team;
    formTeam.alt = formTeam.abbreviation + '-logo';

    if (this.team?.teamId) {
      this.teamService.updateTeam(this.team.teamId, formTeam);
      this.updated.emit(formTeam);
    } else {
