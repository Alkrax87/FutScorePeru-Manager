...
@Component({
  selector: 'app-manager-update-modal',
      ...
      <div class="bg-white p-5 rounded-xl w-full max-w-sm">
        ...
        <div class="flex gap-2 justify-end">
          ...
          <button (click)="onEdit()" class="bg-night hover:bg-neutral-800 text-white border-neutral-200 rounded-lg px-5 py-1.5 font-semibold text-sm">
            <fa-icon [icon]="Add"></fa-icon> Update Manager
          </button>
        </div>
      </div>
    </div>
...
export class ManagerUpdateModalComponent {
  constructor(private teamsService: TeamsApiService) {}

  @Input() editedManager!: Manager;
  ...
  ngOnInit() {
    this.manager = this.editedManager;
